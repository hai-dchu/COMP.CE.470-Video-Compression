...
class FixedPoint {
   public:
    ...
    [[nodiscard]] FixedPoint change_precision(int new_decimal_digits) const {
        int tmp_decimal_digits = new_decimal_digits >= 1 ? new_decimal_digits - (m_decimal_digits != 0) : 0;
        int new_m_value = m_value << tmp_decimal_digits;
        return FixedPoint(new_m_value, new_decimal_digits);
    }
